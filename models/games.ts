...
async function fetchByDate(date: string): Promise<GameData[]> {
  ...
  return data.response.map((game) => ({
    ...
    country: { name: game.country.name, flag: game.country.flag },
    league: { name: game.league.name, logo: game.league.logo },
    teams: {
  ...
  return (data?.[0]?.sports?.[0] ?? []).flatMap((day) =>
    day.sports.flatMap((sport) =>
      sport.leagues.flatMap((league) =>
        [...(league.oldFixtures ?? []), ...(league.fixtures ?? [])]
          .filter((f) => f.home_team && f.visiting_team)
          .map((f) => ({
            id: f.fixture_id,
            date: f.date,
            homeTeam: f.home_team as string,
            visitingTeam: f.visiting_team as string,
            league: f.league ?? "",
