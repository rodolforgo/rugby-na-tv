...
export default function GameRow({ game, isLoggedIn, userId, isAdmin, onVote, onVoteSettled }: Props) {
  ...
  return (
    <>
      <div
        className={`py-2 px-3 text-sm min-h-[72px] flex flex-col justify-center${hasOfficialChannels || hasCommunityBroadcast ? " bg-base-200" : ""}`}
      >
...
function ChannelPill({ channel, onVote, isPending }: ChannelPillProps) {
  return (
    <div className="flex items-stretch rounded-lg border border-base-300 overflow-hidden text-xs">
      <span className="px-2 py-1 text-base-content/85 bg-base-100 border-r border-base-300">{channel.name}</span>
      <button
        type="button"
        disabled={isPending}
